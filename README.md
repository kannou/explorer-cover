...
ブックマークとドライブは、左クリックで現在のペインの選択中タブに開きます。ホイールクリックは現在のペインに新規タブを追加し、そのタブで開きます。

設定画面の「ブックマーク・ドライブを新規タブで開くマウスボタン」で、中ボタン・右ボタン・サイドボタン1・サイドボタン2・無効を選べます。保存後すぐに反映されます。右ボタンを割り当てた場合、名前変更・削除のメニューはブックマークにキーボードでフォーカスを移し、Shift+F10で表示できます。

従来形式の`mouse.json`では`openBookmarkInNewTabButton`に`middle`、`right`、`xButton1`、`xButton2`、`none`を指定できます。省略時は`middle`です。この設定はブックマークとドライブの両方に適用されます。
