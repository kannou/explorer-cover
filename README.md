...
5. タブ・QuickLookに加え、サイドビューから直前のタブへ移動する動作とブックマーク編集を試します。[開発計画](PLAN.md)を見直し、次は保存・復元（6b）へ進みます。
...
設定画面は段階7で追加予定です。現段階ではJSONを編集してアプリを再起動します。プロジェクト内のサンプルを使う場合は、次の環境変数で指定できます。割り当てはタブ見出しだけに適用し、一覧のシェル操作や×ボタン・Ctrl+Wは維持します。
...
- ブックマークの追加・編集とサイドビュー幅も起動中のみ保持します。保存・復元は次の段階6bで追加します。
