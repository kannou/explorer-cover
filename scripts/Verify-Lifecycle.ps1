...
$dll = Join-Path $root 'src\ExplorerAlt\bin\Release\net10.0-windows\ExplorerAlt.dll'
$sdk = Join-Path $root '.tools\dotnet\dotnet.exe'
for ($cycle=1; $cycle -le 3; $cycle++) {
 $log = Join-Path $root "artifacts\lifecycle-$cycle.log"
 $env:EXPLORER_ALT_LOG = $log
 ...
 do {
  ...
  $window = [System.Windows.Automation.AutomationElement]::RootElement.FindAll([System.Windows.Automation.TreeScope]::Children,$all) | Where-Object { $_.Current.ProcessId -eq $app.Id -and $_.Current.Name -like 'Explorer Alt*' } | Select-Object -First 1
...
Remove-Item Env:\EXPLORER_ALT_LOG
