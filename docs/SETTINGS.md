...
- タブ見出しを閉じるボタンは中クリックが初期値。右クリック、サイドボタン1／2、無効にも変更できる。割り当てはタブ見出し上だけに適用し、左クリックの選択・ドラッグ、×ボタン、Ctrl+Wは維持する。
- ブックマークとドライブを新しいタブで開くボタンは中クリックが初期値。右クリック、サイドボタン1／2、無効を選べる。左クリックは選択中タブで開く。
- 右クリックを新規タブに割り当てた場合、ブックマークの編集メニューは項目へキーボードフォーカスを移し、Shift+F10で開く。
...
以下は実装時の検証記録。最新の利用方法は前述の「使い方」と「マウス操作」を参照。
